...
class CoursesDB:
    ...
    def build_tables(self):
        ...
        ;"""
        self.curs.execute(sql) # REFERENCES tSchool(school)
        ...
        CREATE TABLE tRaceResult (
            ...
            -- race_id INTEGER REFERENCES tRace(race_id),  -- SWITCH THIS BACK ONCE tSCHOOL IS FIXED!!!!!!!!!!!!!!!!!!!!!!!!!!
            race TEXT NOT NULL,                            -- SWITCH THIS BACK ONCE tSCHOOL IS FIXED!!!!!!!!!!!!!!!!!!!!!!!!!!
            time TEXT NOT NULL CHECK(time LIKE '__:__._'),                 
    ...
    #     '''
    #     check if order_id exists for this combo
    #     add order_id if not
    #     !ARIS
    #     '''
    #     sql_check = "SELECT race_id FROM tRace WHERE race LIKE ? AND date LIKE ? AND distance LIKE ?;"
    #     x = pd.read_sql(sql_check, self.conn, params = (cust_id, date))
        
    #     # if not, create it (run an INSERT)
    #     if len(x) == 0:
    #         sqlite3.register_adapter(np.int64, lambda val: int(val))
    #         sql_insert = "INSERT INTO tOrder (cust_id, date) VALUES (?, ?);"
    #         self.curs.execute(sql_insert, (cust_id, date))
    #     x = pd.read_sql(sql_check, self.conn, params = (cust_id, date))
    #     # return it   
    #     order_id = x.iloc[0,0]
    #     return order_id
    ...
    def load_results(self, url, gender = 'women', drop_dnf=True, drop_dns=True):
        ...
        for i, row in enumerate(frame.to_dict(orient='records')):
            ...
            #race_id = self.get_race_id(row['race'], row['date'])
            # fill in tables
            row['runner_id'] = runner_id
            try:
                sql = '''
                INSERT INTO tRaceResult (runner_id, race, time, place) VALUES (:runner_id, :COURSE, :TIME, :PL)
                ;'''
